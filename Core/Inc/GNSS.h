...
typedef struct {
	uint8_t minFixmode; // minimal fixMode: 2 - 2d fix mode
	uint8_t firstFix; // first fix flag
	uint8_t oldFixmode; // fix mode on previous data
	float oldHacc;
	float maxHacc;
	float maxSpeed;
...
GNSS_Filter GPS_Filter;
GEO_Point pointFirst;
GEO_Point pointCurrent;
GEO_Point pointOld;
